...
const ExplorePage = () => {
  ...
  const filteredProjects = activeFilter === 'Best results' ? projects.filter(project => project.type === 'Reel') : projects.filter(project => project.industry === activeFilter);
  ...
  const renderGridItem = (project: any, index: number) => {
    ...
    if (isReel) {
      return <div key={project.id} onClick={() => setSelectedProject(project)} className="row-span-2 bg-gray-900 cursor-pointer hover:opacity-80 transition-opacity relative group overflow-hidden rounded-sm">
          <video ref={el => videoRefs.current[project.id] = el} src={project.videoUrl} poster={project.thumbnail} className="w-full h-full object-cover" loop muted playsInline onLoadedData={() => {
          ...
          <div className="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-30 transition-all duration-200 flex items-center justify-center">
            ...
            toggleMute(project.id);
              ...
              {mutedVideos.has(project.id) ? <VolumeX size={16} className="text-white" /> : <Volume2 size={16} className="text-white" />}
            </button>
    ...
    return <div key={project.id} onClick={() => setSelectedProject(project)} className="aspect-square bg-gray-900 cursor-pointer hover:opacity-80 transition-opacity relative group overflow-hidden rounded-sm">
  ...
  return <div className="max-w-md mx-auto">
      ...
      <div className="px-4 py-3 border-b border-gray-800">
        <div className="flex space-x-2 overflow-x-auto">
          {filters.map(filter => <button key={filter} onClick={() => setActiveFilter(filter)} className={`px-4 py-2 rounded-full text-sm font-medium whitespace-nowrap transition-colors ${activeFilter === filter ? 'bg-yellow-400 text-black' : 'bg-gray-800 text-gray-300 hover:bg-gray-700'}`}>
              {filter}
            </button>)}
        </div>
      </div>
      ...
      {/* Project Modal */}
      {selectedProject && <div className="fixed inset-0 bg-black bg-opacity-90 z-50 flex items-center justify-center p-4">
          <div className="bg-gray-900 rounded-lg max-w-md w-full max-h-[80vh] overflow-y-auto">
            ...
            <div className="p-4">
              {selectedProject.type === 'Reel' ? <video src={selectedProject.videoUrl} poster={selectedProject.thumbnail} className="w-full aspect-video object-cover rounded-lg mb-4" controls loop /> : <img src={selectedProject.thumbnail} alt={selectedProject.title} className="w-full aspect-video object-cover rounded-lg mb-4" />}
