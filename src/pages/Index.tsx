  ...
  return <div className="min-h-screen bg-black text-white">
      ...
      {!showAddPage && <header className="sticky top-0 z-50 bg-black border-b border-gray-800 px-4 py-3">
          <div className="max-w-md mx-auto flex items-center justify-between">
            <h1 className="text-[ebe01] text-[#edbe01] font-extrabold"><span className="font-semibold text-deep-white">Social </span>Hive</h1>
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-black border-t border-gray-800 px-4 py-2">
        <div className="max-w-md mx-auto flex justify-around">
          ...
          <button onClick={handleAddClick} className={`p-3 rounded-lg transition-colors ${showAddPage ? 'text-yellow-400' : 'text-gray-400 hover:text-yellow-400'}`}>
            <Plus size={24} />
          </button>
          <button onClick={() => {
          setActiveTab('reels');
